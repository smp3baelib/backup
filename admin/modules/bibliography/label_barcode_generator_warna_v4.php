...
if (isset($_GET['action']) AND $_GET['action'] == 'print') {
  ...
  $callnumb_color = array(0=>"#FFE900",1=>"#d50000",2=>"#e65100",3=>"#ffff00",4=>"#05a818",5=>"#03a9f4",6=>" #9c27b0",7=>"#4e342e",8=>"#bdbdbd",9=>"#000");
  $text_color = array(0=>"#000",1=>"#FFF",2=>"000",3=>"#000",4=>"#000",5=>"#000",6=>"#FFF",7=>"#fff",8=>"#000",9=>"#000");
  ...
  $html_str .= '.box{width: 10cm; height: '.$tinggi.'cm;  border: solid '.$border.'px grey;}'."\n";
  $html_str .= '.header{  text-align: left;  height: auto; width: 8cm; padding: 0.4cm; }'."\n";
  $html_str .= '.barcode{  width: '.$barcode.'cm;  height: '.$tinggi.'cm;}'."\n";
  $html_str .= '.callNum{ font-size: 16px; text-align: center;  position: relative;  top: -'.$tinggi.'cm;  padding-top: 10px;}'."\n";
  $html_str .= '.cc{  height: '.$barcode.'cm;  width:  '.$tinggi.'cm; position: absolute; }'."\n";
  $html_str .= '.cw{  width:  '.$tinggi.'cm;  height:  '.$barcode.'cm; position: absolute; }'."\n";
  $html_str .= '.r > .barcode{ margin-right: 0px;}'."\n";
  $html_str .= '.r > .callNum{  margin-left: 0px; width: 3cm;float: left;}'."\n";
  $html_str .= '.l > .barcode{  margin-left: 0px;}'."\n";
  $html_str .= '.l > .callNum{  margin-left: 0px; width: 3cm;float: right;}'."\n";
  $html_str .= '.img_code{ width: 100%; height:75%; padding:-5px; border:0px; }'."\n";
  $html_str .= '.title{  font-size:8pt; padding-left: 0.5cm;width: 8cm;}'."\n";
  $html_str .= '</style>'."\n";
  $html_str .= '</head>'."\n";
  $html_str .= '<body>'."\n";
  ...
  foreach ($chunked_barcode_arrays as $barcode_rows) {
    ...
    foreach ($barcode_rows as $barcode) {
       ...
       $html_str .= '<img class="img_code" src="'.SWB.IMG.'/barcodes/'.str_replace(array(' '), '_', $barcode[1]).'.png"/><div class="header" style="text-transform: uppercase;font-weight: bold;z-index:-2;">'.($sysconf['print']['barcode']['barcode_header_text']?$sysconf['print']['barcode']['barcode_header_text']:$sysconf['library_name']).'</div></div>'."\n";
       //$html_str .=  '<div class="header" style="text-transform: uppercase;font-weight: bold;z-index:-2;">'.($sysconf['print']['barcode']['barcode_header_text']?$sysconf['print']['barcode']['barcode_header_text']:$sysconf['library_name']).'</div>'."\n";
       $html_str .= '</div>'."\n";

       $html_str .=  '<div class="callNum" style="color:'.$callnumb_color[substr($barcode[3],0,1)].';font-weight: bold;">'."\n";
...
<div class="menuBoxInner printIcon">
  ...
  <div class="sub_section">
	  <div class="btn-group">
      <a target="blindSubmit" href="<?php echo MWB; ?>bibliography/label_barcode_generator_warna_v4.php?action=clear" class="notAJAX btn btn-default"><i class="glyphicon glyphicon-trash"></i>&nbsp;<?php echo __('Clear Print Queue'); ?></a>
      <a target="blindSubmit" href="<?php echo MWB; ?>bibliography/label_barcode_generator_warna_v4.php?action=print" class="notAJAX btn btn-success"><i class="glyphicon glyphicon-print"></i>&nbsp;<?php echo __('Print Barcodes for Selected Data');?></a>
    ...
    <form name="search" action="<?php echo MWB; ?>bibliography/label_barcode_generator_warna_v4.php" id="search" method="get" style="display: inline;"><?php echo __('Search'); ?> :
...
if ($sysconf['index']['type'] == 'index' || ($sysconf['index']['type'] == 'sphinx' && file_exists(LIB.'sphinx/sphinxapi.php'))) {
  ...
  $datagrid->setSQLColumn('item.item_code',
    ...
    'biblio.last_update AS \''.__('Last Update 2').'\'');
}
...
if (isset($_GET['keywords']) AND $_GET['keywords']) {
  ...
  $biblio_list = new biblio_list($dbs, 21);
...
$datagrid_result = $datagrid->createDataGrid($dbs, $table_spec, 21, $can_read);
if (isset($_GET['keywords']) AND $_GET['keywords']) {
