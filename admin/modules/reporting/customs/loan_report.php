...
$page_title = 'Laporan Statistik Pinjaman';
$reportView = false;
...
?>
    ...
    <fieldset>
    <div class="per_title">
        <h2><?php echo __('Laporan Statistik Pinjaman'); ?></h2>
    </div>
    <div class="infoBox">
    ...
    <div id="filterForm">
        <div class="divRow">
            ...
            $mtype_options = array();
            $mtype_options[] = array('0', __('Pilih'));
...
<?php
    ...
    */
    ...
    // header
    $output .= '<tr>';
    $output .= '<td class="dataListHeaderPrinted" style="color: #255F24; font-weight: bold;">'.__('Member Type').'</td>';
    foreach ($months as $month_num => $month) {
        $total_month[$month_num] = 0;
        $output .= '<td class="dataListHeaderPrinted" style="color: #255F24; font-weight: bold;">'.$month.'</td>';
    }
    $output .= '</tr>';
    ...
    if(isset($_GET['member_type']) AND !empty($_GET['member_type'])){
        ...
        $outloan  = "";
        $outloan .= '{';
        $outloan .= 'fillColor : "#'.generateRandomColors().'",';
        $outloan .= 'title: "'.$member_type.'",';
        $outloan .= 'data:[';
        foreach ($months as $month_num => $month) {
            $sql_str = "SELECT COUNT(loan_id) FROM loan AS lo
                INNER JOIN (member AS m LEFT JOIN mst_member_type AS mt ON m.member_type_id=mt.member_type_id) ON m.member_id=lo.member_id
                WHERE m.member_type_id=$id AND lo.loan_date LIKE '$selected_year-$month_num-%'";
    ...
    $r = 1;
    // count library member loan each month
    ...
    $output .= '<tr>';
    $output .= '<td class="dataListHeaderPrinted" style="color: #255F24; font-weight: bold;">'.__('Total Peminjaman/month').'</td>';
    foreach ($months as $month_num => $month) {
        $output .= '<td class="dataListHeaderPrinted" style="color: #255F24; font-weight: bold;">'.$total_month[$month_num].'</td>';
    }
    }
    $output .= '</tr>';
    ...
    // print out
    echo '<div class="printPageInfo">Total Peminjam Perpustakaan Tahun <strong>'.$selected_year.'</strong> <a class="printReport" onclick="window.print()" href="#">'.__('Print Current Page').'</a></div>'."\n";
