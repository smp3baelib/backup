...
$page_title = 'Members Loan Report';
$reportView = false;
$num_recs_show = 20;
...
?>
    <!-- filter --><fieldset>
    <div class="per_title">
      <h2><?php echo __('Laporan Terbanyak Pinjam / Kunjungan'); ?></h2>
    </div>
    <div class="infoBox">
...
<?php
 ...
 // table spec
    $table_spec = 'loan AS l LEFT JOIN member AS m
    ...
    $reportgrid = new report_datagrid();
	 //loan filter
        ...
        $loan_filter .= ' AND (TO_DAYS(loan_date) BETWEEN TO_DAYS(\''.$_GET['startDate'].'\') AND
        TO_DAYS(\''.$_GET['untilDate'].'\'))';}
    //fines filter
	    if (isset($_GET['startDate']) AND isset($_GET['untilDate'])) {
        $fines_filter .= ' AND (TO_DAYS(fines_date) BETWEEN TO_DAYS(\''.$_GET['startDate'].'\') AND
        TO_DAYS(\''.$_GET['untilDate'].'\'))';}
		
	//visitor filter	
	    if (isset($_GET['startDate']) AND isset($_GET['untilDate'])) {
        $visitor_filter .= ' AND (TO_DAYS(checkin_date) BETWEEN TO_DAYS(\''.$_GET['startDate'].'\') AND
        TO_DAYS(\''.$_GET['untilDate'].'\'))';}
		
    $reportgrid->setSQLColumn(
	    'm.member_id AS \''.__('Member ID').'\'',
		'm.member_name AS \''.__('Member Name').'\'',
        'COUNT(l.member_id) AS \''.__('Jumlah Peminjaman').'\'',
		'(SELECT COUNT(member_id) FROM visitor_count  WHERE member_id=m.member_id '.$visitor_filter.') AS \''.__('Jumlah Kunjungan').'\'',
		'(SELECT COUNT(return_date) FROM loan  WHERE return_date!="NULL" AND return_date>due_date AND member_id=l.member_id '.$loan_filter.') AS \''.__('Keterlambatan').'\'',
