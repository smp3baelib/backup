...
?>
    ...
    <fieldset>
    <div class="per_title">
        <h2><?php echo __('Laporan & Grafik Pengunjung '); ?></h2>
    </div>
    <div class="infoBox">
...
<?php
    ...
    foreach ($member_types as $id => $member_type) {
        ...
        foreach ($months as $month_num => $month) {
            ...
            if ($visitor_d[0] < 1000) {
                ...
                $a = '<td class="'.$row_class.'"><strong style="font-size: 1.5em;">'.$visitor_d[0].'</strong></td>';
            } else {
    ...
    // print out
    echo '<div class="printPageInfo">Visitor Count Report for year <strong>'.$selected_year.'</strong> <a class="printReport" onclick="window.print()" href="#">'.__('Print Current Page').'</a>';
    //add heru subekti (heroe_soebekti@yahoo.co.id)
    echo '<a class="printReport" onclick="window.open(\''.MWB.'reporting/plot_report.php?chart=visitor_statistic&year='.$selected_year.'\', \'myWindow\', \'status=0, width=950, height=600, noborder=0\')">'.__('Show in chart/plot').'</a></div>'; 
    echo $output;
