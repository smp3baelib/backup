...
?>
    ...
    <div id="filterForm">
        <div class="divRow">
            ...
            <div class="divRowLabel"><?php echo __('Type Pengunjung'); ?></div>
            <div class="divRowContent">
            <?php
            $vmode_chbox[0] = array('0', __('GLOBAL'));
            $vmode_chbox[1] = array('1', __('UNIK'));
            echo simbio_form_element::radioButton('vmode', $vmode_chbox, 'ALL');
            ?>            
        </div>
    </div>
...
<?php
    ...
    //filter visitor mode add heru
    $vmode = '';
    if (isset($_GET['vmode']) AND !empty($_GET['vmode']) AND $_GET['vmode']==1) {
        $vmode .= ' DISTINCT ';
    }    
    //
    // query visitor data to database
    $_visitor_q = $dbs->query("SELECT SUBSTRING(`checkin_date`, 9, 2) AS `mdate`, COUNT( $vmode member_id) AS `vtotal` FROM `visitor_count` WHERE `checkin_date` LIKE '$selected_year-$selected_month%' GROUP BY DATE(`checkin_date`)");
