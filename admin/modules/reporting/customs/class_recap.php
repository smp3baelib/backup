...
<?php
    ...
    // print out
    echo '<div class="printPageInfo">'.__('Title and Collection Recap by').' <strong>'.$recapby.'</strong> <a class="printReport" onclick="window.print()" href="#">'.__('Print Current Page').'</a><a href="../xlsoutput.php" class="button">'.__('Export to spreadsheet format').'</a>';
    //add heru subekti (heroe_soebekti@yahoo.co.id)
    echo '<a class="printReport" onclick="window.open(\''.MWB.'reporting/plot_report.php?chart=reccap&reccap_by='.$recapby.'\', \'myWindow\', \'status=0, width=950, height=600, noborder=0\')">'.__('Show in chart/plot').'</a></div>'."\n"; 
        echo $output;
