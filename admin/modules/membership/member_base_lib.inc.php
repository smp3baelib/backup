...
class member
{
    ...
    public function __construct($obj_db, $str_member_id)
    {
	    ...
	    // Query to Check if member have loan
        // Modification by Drajat Hasan
        $_member_q = $this->obj_db->query(sprintf("SELECT m.*,mt.*,l.member_id,l.is_return FROM member AS m JOIN mst_member_type AS mt ON m.member_type_id=mt.member_type_id JOIN loan AS l ON l.member_id = m.member_id WHERE m.member_id='%s' AND l.is_return = 0", $str_member_id));
	    // else
        $_member_qu = $this->obj_db->query(sprintf("SELECT m.*,mt.* FROM member AS m JOIN mst_member_type AS mt ON m.member_type_id=mt.member_type_id WHERE m.member_id='%s'", $str_member_id));
        // end modification 
        ...
        if ($_member_q->num_rows > 0) {
	        ...
	        $this->is_return = $_member_d['is_return'];
        ...
        } elseif ($_member_q->num_rows == 0 && $_member_qu->num_rows > 0) {
            // set the is_member property flag to TRUE
            $this->is_member = true;
            $_member_d = $_member_qu->fetch_assoc();
            // assign database value to class properties
	        $this->is_return = null;
            $this->member_id = $_member_d['member_id'];
            $this->member_name = $_member_d['member_name'];
