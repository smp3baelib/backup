...
/* Other Message Management section */
...
<div class="menuBoxInner masterFileIcon">
	<div class="per_title">
	    <h2><?php echo __('Other Messages'); ?></h2>
  </div>
...
/* RECORD OPERATION */
    ...
    if (isset($_POST['itemID']) AND !empty($_POST['itemID']) AND isset($_POST['itemAction'])) {
        ...
        foreach ($_POST['itemID'] as $itemID) {
            ...
            if (!$sql_op->delete('other_inbox', 'ID='.$itemID)) {
                $error_num++;
            }
        }  
    ...
    // table spec
    $table_spec = 'other_inbox AS i LEFT JOIN member AS m ON m.member_phone=i.DestinationNumber';

    // create datagrid
    $datagrid = new simbio_datagrid();
        if ($can_write AND $can_read ) { 
        $datagrid->setSQLColumn('i.ID',
                                'i.DestinationNumber AS \''.__('Reply').'\'',
                                'i.DestinationNumber AS \''.__('Sender').'\'',
   //     						'm.member_name AS \''.__('Name').'\'',
        						'i.SendingDateTime AS \''.__('Date/Time').'\'',
        						'i.TextDecoded AS \''.__('Message').'\'');
    }
    else{
        $datagrid->setSQLColumn('i.DestinationNumber AS \''.__('Sender').'\'',
   //                           'm.member_name AS \''.__('Name').'\'',
                                'i.SendingDateTime AS \''.__('Date/Time').'\'',
                                'i.TextDecoded AS \''.__('Message').'\'');        
    }
    
    $datagrid->setSQLorder('i.SendingDateTime DESC');
    ...
    function showLink($obj_db, $array_data)
        {   $_output = '<a href="'.MWB.'sms_gateway/compose.php?p='.urlencode($array_data[1]).'"><div class="editLink"></div></a>';
            return $_output;
        }
    if($can_write AND $can_read){    
    $datagrid->modifyColumnContent(1, 'callback{showLink}');  }     
    // set delete proccess URL
    $datagrid->chbox_form_URL = $_SERVER['PHP_SELF'];
    //$datagrid->column_width = array(0 =>'5%', 1 => '15%', 2 => '15%',3=>'75%');
