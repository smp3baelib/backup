...
/* RECORD OPERATION */
    ...
    if (isset($_POST['itemID']) AND !empty($_POST['itemID']) AND isset($_POST['itemAction'])) {
        ...
        foreach ($_POST['itemID'] as $itemID) {
            ...
            if (!$sql_op->delete('sentitems', 'ID='.$itemID)) {
                $error_num++;
            }
        }  
...
<?php
    // table spec
    $table_spec = 'sentitems AS i LEFT JOIN member AS m ON m.member_phone=i.DestinationNumber';
    ...
    if ($can_write AND $can_read ) {    
        $datagrid->setSQLColumn('i.ID',
                                ...
                                'IF(i.Status="SendingOKNoReport","'.__('Sending').'","'.__('<div class=\'label label-info\'>Failed, Check Voucher</div>').'") AS \''.__('Status').'\'',
    ...
    //$datagrid->column_width = array(0 =>'5%', 1 => '10%', 2 => '12%', 3 => '15%', 4 => '70%');
