...
/* RECORD OPERATION */
    ...
    if (isset($_POST['itemID']) AND !empty($_POST['itemID']) AND isset($_POST['itemAction'])) {
        ...
        foreach ($_POST['itemID'] as $itemID) {
            ...
            if (!$sql_op->delete('inbox', 'ID='.$itemID)) {
                $error_num++;
            }
        }  
    ...
    /* INBOX LIST */
    // table spec
    $table_spec = 'inbox AS i LEFT JOIN member AS m ON m.member_phone=i.SenderNumber';

    // create datagrid
    $datagrid = new simbio_datagrid();
    if ($can_write AND $can_read ) {      
    $datagrid->setSQLColumn('i.ID',
                                'i.SenderNumber AS \''.__('Sender').'\'',
        						'm.member_name AS \''.__('Name').'\'',
        						'i.ReceivingDateTime AS \''.__('Date/Time').'\'',
                                'i.Processed AS \''.__('Processed').'\'',
        						'i.TextDecoded AS \''.__('Message').'\'');
    }else{
    $datagrid->setSQLColumn('i.SenderNumber AS \''.__('Sender').'\'',
                                'm.member_name AS \''.__('Name').'\'',
                                'i.ReceivingDateTime AS \''.__('Date/Time').'\'',
                                'i.Processed AS \''.__('Processed').'\'',
                                'i.TextDecoded AS \''.__('Message').'\'');        
    ...
    $datagrid->setSQLorder('i.ReceivingDateTime DESC');
    ...
    if (isset($_GET['keywords']) AND $_GET['keywords']) {
       ...
       $datagrid->setSQLCriteria("i.SenderNumber LIKE '%$keywords%' OR m.member_name LIKE '%$keywords%'");
    }
    ...
    $datagrid->column_width = array(0 =>'10%', 1 => '15%', 2 => '15%',3 => '5%', 4 => '75%');

    // put the result into variables
    $datagrid_result = $datagrid->createDataGrid($dbs, $table_spec, 25,($can_read AND $can_write));
    if (isset($_GET['keywords']) AND $_GET['keywords']) {
